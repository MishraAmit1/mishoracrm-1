    ...
    <div class="pf-body">
        ...
        <div class="field" style="flex-direction:row;align-items:center;gap:10px;padding:10px 12px;background:var(--bg-elevated);border-radius:var(--r-sm)">
            <input type="checkbox" name="is_package" value="1" id="isPackage"
                   {{ old('is_package', $service->is_package) ? 'checked' : '' }} onchange="onPackageToggle()"
        ...
        <div class="field" id="packageComponentsField" style="{{ old('is_package', $service->is_package) ? '' : 'display:none' }}">
            ...
            <div style="display:flex;flex-direction:column;gap:6px;max-height:200px;overflow-y:auto;padding:10px 12px;background:var(--bg-input);border:1.5px solid var(--border-default);border-radius:var(--r-sm)">
                ...
                @php $oldSelected = old('component_service_ids', $selectedComponentIds); @endphp
                ...
                @empty
                <span style="font-size:12.5px;color:var(--text-400)">No standalone services available.</span>
                @endforelse
            </div>
            <span style="font-size:11.5px;color:var(--text-400)">This is for reference only — set the Rate/Tax/Billing Cycle below yourself.</span>
        </div>
        ...
        <div class="field">
            ...
            <input type="text" name="category" class="fi" value="{{ old('category', $service->category) }}" placeholder="e.g. Consulting, Repairs"/>
        ...
        <div class="field">
            ...
            <input type="number" name="total_quantity" class="fi" min="1"
                   value="{{ old('total_quantity', $service->total_quantity) }}" placeholder="e.g. 10"/>
