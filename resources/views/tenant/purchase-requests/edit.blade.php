@extends('layouts.app')
@section('title', 'Edit Purchase Request — ' . $purchaseRequest->number)
    ...
    <div class="page-head">
        <div>
            <div style="font-size:12px;color:var(--text-300);margin-bottom:4px;display:flex;align-items:center;gap:5px">
                ...
                <a href="{{ route('tenant.purchase-requests.show',$purchaseRequest->id) }}" style="color:var(--text-300);text-decoration:none">{{ $purchaseRequest->number }}</a>
                <span style="opacity:.4">›</span>
                <span>Edit</span>
            </div>
            <div class="page-title">Edit Purchase Request</div>
        </div>
        <a href="{{ route('tenant.purchase-requests.show',$purchaseRequest->id) }}" class="btn btn-secondary">
            <i class="ti ti-eye" style="font-size:14px"></i> View
        </a>
    </div>

    <form method="POST" action="{{ route('tenant.purchase-requests.update', $purchaseRequest->id) }}" novalidate>
        @csrf @method('PUT')
        <div class="pf-layout">
            <div>
                    ...
                    <div class="pf-section">
                        ...
                        <div class="pf-grid">
                            <div class="pf-field">
                                ...
                                <input type="text" class="pf-input" value="{{ $purchaseRequest->number }}" readonly
                            ...
                            <div class="pf-field">
                                ...
                                <input type="date" name="date" class="pf-input {{ $errors->has('date')?'is-err':'' }}"
                                       value="{{ old('date', $purchaseRequest->date?->format('Y-m-d')) }}" required/>
                                @error('date')<span class="pf-err">{{ $message }}</span>@enderror
                            </div>
                            <div class="pf-field span-full">
                                <label class="pf-label">Department</label>
                                <select name="department_id" class="pf-input">
                                    <option value="">— Select Department (optional) —</option>
                                    @foreach($departments as $d)
                                    <option value="{{ $d->id }}" {{ old('department_id', $purchaseRequest->department_id) == $d->id ? 'selected':'' }}>{{ $d->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                ...
                <div class="pf-card">
                    <div class="pf-section">
                        ...
                        <div class="pf-grid">
                            <div class="pf-field span-full">
                                <textarea name="reason" class="pf-input pf-area" rows="3">{{ old('reason', $purchaseRequest->reason) }}</textarea>
                    ...
                    <div class="pf-footer">
                        ...
                        <div style="display:flex;gap:8px">
                            <a href="{{ route('tenant.purchase-requests.show',$purchaseRequest->id) }}" class="btn btn-secondary">Cancel</a>
            ...
            <div class="pf-sidebar">
                <div class="pf-sc">
                    <div class="pf-sc-title">Request Info</div>
                    <div class="pr-number">{{ $purchaseRequest->number }}</div>
                    <div style="font-size:12px;color:var(--text-300);margin-top:6px">Status: {{ ucfirst($purchaseRequest->status) }}</div>
                </div>
            </div>
        </div>
    </form>
...
window.addItemRow = function(name='', description='', qty=1, note='', productId=''){
    ...
    tr.innerHTML = `
        ...
        <td data-label="Qty">
            <input type="number" name="items[${i}][quantity]" class="item-input" value="${qty}" min="0.01" step="0.01" required/>
        </td>
        <td data-label="Note">
...
const items = @json(old('items', $purchaseRequest->items ?? []));
if(items && items.length){
    items.forEach(item => addItemRow(item.name||'', item.description||'', item.quantity||1, item.reason||'', item.product_id||''));
} else {
    addItemRow();
}
