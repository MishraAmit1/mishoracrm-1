@extends('layouts.app')
@section('title', 'New Task Template')
...
<div class="tt-form-wrap">
    ...
    <div class="page-head">
        <div>
            ...
            <div class="page-title">New Task Template</div>
        </div>
    </div>

    <form method="POST" action="{{ route('tenant.task-templates.store') }}">
        @csrf
        <div class="tt-card">

            <div class="tt-field">
                <label class="tt-label">Template Name *</label>
                <input type="text" name="name" class="tt-input" value="{{ old('name') }}" placeholder="e.g. New Lead Onboarding" required maxlength="255">
            ...
            <div class="tt-field">
                ...
                <textarea name="description" class="tt-input" rows="3" placeholder="Task description that gets pre-filled...">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="tt-field">
                ...
                <select name="default_priority" class="tt-input">
                    ...
                    <option value="{{ $key }}" {{ old('default_priority', 'medium') === $key ? 'selected' : '' }}>{{ $p['label'] }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="tt-field">
                ...
                <input type="text" name="default_tags" class="tt-input" value="{{ old('default_tags') }}" placeholder="e.g. onboarding, urgent (comma separated)">
            </div>

            <div class="tt-field">
                <label class="tt-label">Checklist Items</label>
                <textarea name="checklist_items" class="tt-input" rows="6" placeholder="One checklist item per line, e.g.&#10;Send welcome email&#10;Schedule kickoff call&#10;Share onboarding docs">{{ old('checklist_items') }}</textarea>
        ...
        <div class="tt-footer">
            ...
            <button type="submit" class="btn btn-primary"><i class="ti ti-plus"></i> Create Template</button>
        </div>
    </form>
