...
<div class="page-head">
    <div>
        <div style="font-size:13px;color:var(--text-300);margin-bottom:4px">
            ...
            <a href="{{ route('tenant.staffs.show', $staff->id) }}" style="color:var(--text-300);text-decoration:none">
                {{ $staff->user->name }}
            </a>
    ...
    <a href="{{ route('tenant.staffs.show', $staff->id) }}" class="btn btn-secondary">← Back</a>
</div>

<div class="form-card">
    <form method="POST" action="{{ route('tenant.staffs.update', $staff->id) }}" novalidate>
        @csrf
        @method('PUT')

        {{-- ── Account section (config driven) ────────────────── --}}
        ...
        <div class="form-section">
            ...
            {{-- Password optional note --}}
            <div class="pw-note">
                🔒 Leave password fields blank to keep existing password unchanged.
            </div>
            ...
            <div class="form-grid">
                ...
                @php
                    ...
                    $isRequired = $f['required'] && !in_array($f['name'], ['password','password_confirmation']);
                    $val = old($f['name'], $prefill[$f['name']] ?? '');
                @endphp
                <div class="field {{ ($f['span'] ?? 1) > 1 ? 'span-2' : '' }}">
                    <label class="field-label">
                        {{ $f['label'] }}
                        @if($isRequired)<span class="req">*</span>@endif
                        @if(in_array($f['name'], ['password','password_confirmation']))
                        <span style="font-size:11px;color:var(--text-400);font-weight:400;text-transform:none">(optional)</span>
                        @endif
                    </label>
            ...
            <div class="form-grid">
                ...
                <div class="field">
                    ...
                    <select name="department_id" class="field-input field-select">
                        ...
                        <option value="{{ $dept->id }}"
                            {{ old('department_id', $prefill['department_id']) == $dept->id ? 'selected':'' }}>
                            {{ $dept->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="field">
                    ...
                    <select name="employment_type" class="field-input field-select">
                        ...
                        <option value="{{ $key }}"
                            {{ old('employment_type', $prefill['employment_type']) === $key ? 'selected':'' }}>
                            {{ $et['label'] }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="field {{ ($f['span'] ?? 1) > 1 ? 'span-2' : '' }}">
                    ...
                    <input type="{{ $f['type'] }}" name="{{ $f['name'] }}"
                           class="field-input {{ $errors->has($f['name']) ? 'is-error':'' }}"
                           placeholder="{{ $f['placeholder'] }}"
                           value="{{ old($f['name'], $prefill[$f['name']] ?? '') }}"
        ...
        <div class="form-actions">
            <a href="{{ route('tenant.staffs.show', $staff->id) }}" class="btn btn-secondary">Cancel</a>
