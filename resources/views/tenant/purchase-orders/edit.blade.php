...
<div class="qf">
    ...
    <div class="page-head">
        ...
        <a href="{{ route('tenant.purchase-orders.show',$purchaseOrder->id) }}" class="btn btn-secondary">
    ...
    <form method="POST" action="{{ route('tenant.purchase-orders.update', $purchaseOrder->id) }}" novalidate id="poForm">
        @csrf @method('PUT')
        ...
        <div class="qf-layout">
                    ...
                    <div class="qf-section">
                        ...
                        <div class="qf-grid">
                            <div class="qf-field">
                                ...
                                <input type="text" class="qf-input" value="{{ $purchaseOrder->number }}" readonly
                            ...
                            <div class="qf-field">
                                ...
                                <input type="date" name="date" class="qf-input {{ $errors->has('date')?'is-err':'' }}"
                                       value="{{ old('date', $purchaseOrder->date?->format('Y-m-d')) }}" required/>
                                @error('date')<span class="qf-err">{{ $message }}</span>@enderror
                            </div>
                            <div class="qf-field">
                                <label class="qf-label">Expected Delivery</label>
                    ...
                    <div class="qf-section">
                        ...
                        <div class="qf-grid">
                            <div class="qf-field span-full">
                                <label class="qf-label">Vendor</label>
                                <select name="vendor_id" class="qf-input qf-sel {{ $errors->has('vendor_id')?'is-err':'' }}">
                                    <option value="">— Select Vendor (optional) —</option>
                                    @foreach($vendors as $vd)
                                    <option value="{{ $vd->id }}" {{ old('vendor_id', $purchaseOrder->vendor_id) == $vd->id ? 'selected':'' }}>
                ...
                <div class="qf-card" style="margin-bottom:14px">
                    ...
                    <div class="totals-wrap">
                        <table class="totals-table">
                            <tr>
                                <td>Subtotal</td>
                                <td id="displaySubtotal">₹{{ number_format($purchaseOrder->subtotal ?? 0, 2) }}</td>
                            </tr>
                            <tr>
                                <td>
                                    Discount
                            ...
                            <tr class="grand-total">
                                ...
                                <td id="displayTotal"><strong>₹{{ number_format($purchaseOrder->total ?? 0, 2) }}</strong></td>
                            </tr>
                        </table>
                    </div>

                    <input type="hidden" name="subtotal" id="hiddenSubtotal" value="{{ $purchaseOrder->subtotal ?? 0 }}">
                    <input type="hidden" name="tax_amount" id="hiddenTaxAmount" value="{{ $purchaseOrder->tax_amount ?? 0 }}">
                    <input type="hidden" name="total" id="hiddenTotal" value="{{ $purchaseOrder->total ?? 0 }}">
                </div>
                    ...
                    <div class="qf-section">
                        ...
                        <div class="qf-grid">
                            <div class="qf-field span-full">
                                ...
                                <textarea name="notes" class="qf-input qf-area" rows="3">{{ old('notes', $purchaseOrder->notes) }}</textarea>
                            </div>
                            <div class="qf-field span-full">
                                <label class="qf-label">Terms & Conditions</label>
                                <textarea name="terms" class="qf-input qf-area" rows="4">{{ old('terms', $purchaseOrder->terms) }}</textarea>
                            </div>
                        </div>
                    </div>
                    <div class="qf-footer">
                        ...
                        <div style="display:flex;gap:8px">
                            <a href="{{ route('tenant.purchase-orders.show',$purchaseOrder->id) }}" class="btn btn-secondary">Cancel</a>
            ...
            <div class="qf-sidebar">
                <div class="qf-sc">
                    <div class="qf-sc-title">Purchase Order Info</div>
                    <div class="qp-number">{{ $purchaseOrder->number }}</div>
                    <div class="qp-total" id="sidebarTotal">₹{{ number_format($purchaseOrder->total ?? 0, 2) }}</div>
                    <div style="height:1px;background:var(--border-subtle);margin:12px 0"></div>
                    <div style="font-size:12px;color:var(--text-300)">Status: <strong style="color:var(--text-200)">{{ ucfirst(str_replace('_',' ',$purchaseOrder->status)) }}</strong></div>
                </div>
            </div>
        </div>
    </form>
...
const items = @json(old('items', $purchaseOrder->items ?? []));
if(items && items.length){
    items.forEach(item => addItemRow(item.name||'', item.description||'', item.quantity||1, item.rate||0, item.tax_percent||'', item.product_id||''));
} else {
    addItemRow();
}
