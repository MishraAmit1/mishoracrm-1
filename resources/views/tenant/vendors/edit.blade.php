...
<div class="vf-page">
    ...
    <div class="page-head">
        <div>
            <div style="font-size:12px;color:var(--text-300);margin-bottom:4px;display:flex;align-items:center;gap:5px">
                ...
                <a href="{{ route('tenant.vendors.show',$vendor->id) }}" style="color:var(--text-300);text-decoration:none">{{ $vendor->name }}</a>
                <span style="opacity:.4">›</span>
                <span>Edit</span>
            </div>
            <div class="page-title">Edit Vendor</div>
        </div>
        <div style="display:flex;gap:8px">
            <a href="{{ route('tenant.vendors.show',$vendor->id) }}" class="btn btn-secondary">
    ...
    <form method="POST" action="{{ route('tenant.vendors.update', $vendor->id) }}" novalidate>
        @csrf @method('PUT')
        ...
        <div class="vf-main" style="margin-top:14px">
            <div class="vf-footer">
                ...
                <div style="display:flex;gap:8px">
                    <a href="{{ route('tenant.vendors.show',$vendor->id) }}" class="btn btn-secondary">Cancel</a>
