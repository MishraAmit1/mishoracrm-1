...
@php
    ...
    $statusLabels = \App\Models\Ticket::statuses();
@endphp
...
<div class="page-head">
    <div>
        <div style="font-size:13px;color:var(--text-300);margin-bottom:4px">
            ...
            <span style="margin:0 6px">›</span> Tickets
        </div>
        <div class="page-title">Tickets Report</div>
...
<div class="range-bar">
    ...
    <a href="{{ route('tenant.reports.tickets', ['range'=>$key]) }}" class="range-btn {{ $curRange===$key ? 'active':'' }}">{{ $label }}</a>
    @endforeach
</div>

<div class="kpi-grid">
    <div class="kpi-card"><div class="kpi-label">Total</div><div class="kpi-value">{{ $kpis['total'] }}</div></div>
    <div class="kpi-card"><div class="kpi-label">Open</div><div class="kpi-value" style="color:var(--accent)">{{ $kpis['open'] }}</div></div>
    <div class="kpi-card"><div class="kpi-label">Resolved</div><div class="kpi-value" style="color:var(--green)">{{ $kpis['resolved'] }}</div></div>
    <div class="kpi-card"><div class="kpi-label">Avg Resolution</div><div class="kpi-value">{{ $avgResolutionHours !== null ? round($avgResolutionHours, 1) . 'h' : '—' }}</div></div>
    <div class="kpi-card"><div class="kpi-label">SLA Breached</div><div class="kpi-value" style="color:var(--red)">{{ $kpis['sla_breached'] }}</div></div>
</div>

<div class="two-col">
    <div class="chart-card">
        <div class="chart-head">By Priority</div>
        <div class="chart-body">
            @forelse($byPriority as $priority => $row)
            <div class="row-item"><span class="row-name">{{ $priorityLabels[$priority] ?? ucfirst($priority) }}</span><span class="row-num">{{ $row->count }}</span></div>
            @empty
...
<div class="chart-card">
    <div class="chart-head">Top Staff by Assigned Tickets</div>
    <div class="chart-body">
        @forelse($byStaff as $row)
        <div class="row-item"><span class="row-name">{{ $row->assignee?->name ?? '—' }}</span><span class="row-num">{{ $row->count }}</span></div>
        @empty
        <div style="color:var(--text-400);font-size:13px">No data yet.</div>
        @endforelse
    </div>
